...
実写での精度は、公開データセット[SHEL5K](https://data.mendeley.com/datasets/9rcv8mm682/4)（CC BY 4.0・建設現場等の5,000枚・ヘルメット未着用者のボックスラベルから画像単位GTを導出）で実測しています。均衡サンプル60枚（違反30/クリーン30・シード固定）で「未着用者がいる画像を指摘できるか」を測った結果です。

| 構成 | precision | recall | F1 |
|---|---|---|---|
| **特化チューニング2B probe（`--probe-model`・近日公開）** | **0.903** | **0.933** | **0.918** |
| パイプライン全体・判定=Qwen3-8B（既定） | 0.574 | 0.900 | 0.702 |
| パイプライン全体・判定=Qwen3-14B | 0.512 | 0.700 | 0.592 |
| 参考: 汎用VLM追加確認の単体回答 | 0.569 | 0.967 | 0.716 |

特化チューニング2Bは、SHEL5K trainのボックスラベルから導出した「人数・着用・未着用」カウントでQwen3-VL-2BをLoRA学習したものです（評価60枚は学習から除外）。汎用VLMの過剰カウント（クリーン画像でも「未着用1人」と答えがち）が解消し、9Bの汎用VLMをrecallで同等・precisionで大きく上回ります。`--probe-model`で追加確認だけをこのモデルに差し替えられます。

recall重視のスクリーニング特性です。違反の9割を拾う一方、指摘の約半分は誤報なので、**フラグを人が確認する運用**（巡回写真の一次スクリーニング）に向きます。誤報の主因はVLMがクリーン画像でも「未着用1人」と数えがちなことで、位置の特定まで求めると今度は「全員着用」に振れる（recall 0.07）ことも実測しており、ローカル4bit VLMの弁別能力が現在の上限です。再現手順は `bench/shel5k.py` に同梱しています（モデル差し替えで再実測できます）。
